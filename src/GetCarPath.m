function path = GetCarPath(spawnTime, car, awarenessType, nodeList, liveMap, edgeMatrix, globalEventQueue)
    % Constants
    TIME = 1;
    TYPE = 2;
    NODE_INDEX = 3;
    ...
    eventQueue.insert([spawnTime EXPLORE_NODE car.spawnPoint]);
            ...
            if currentNode == car.goal % Goal found. From here no further exploring is needed.
                ...
                for i = 1 : length(nodeList(currentNode).outgoingNodes)
                    ...
                    if ~notebook(head,EXPLORED)
                        timeToGetThere = tCarsMind + EstimateTravelTime(awarenessType,liveMap(head,currentNode),liveMapInCarsHead(head,currentNode),edgeMatrix(head,currentNode,:));
